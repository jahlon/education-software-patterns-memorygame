...
public class Board {
	 ...
	 */
	public Board(int[] numbers, int rows, int columns, int specialPairs) {
		this.numbers = numbers;
		boxes = new ArrayList<Box>();
		matchesMade = 0;
		this.rows = rows;
		this.columns = columns;
		initializeBoard(specialPairs);
	}
	 ...
	 * @param specialPairs int indicating the number of special pairs in the board
	 */
	private void initializeBoard(int specialPairs) {
		int numBox = 1;
		
		List<Integer> specialBoxes = new ArrayList<Integer>();
		
		Random r = new Random();
		while(specialBoxes.size() < specialPairs) {
			int n = numbers[r.nextInt(numbers.length)];
			if(!specialBoxes.contains(n)) {
				specialBoxes.add(n);
			}
		}
		...
		for (int i=0; i < numbers.length; i++) {
			int n = numbers[i];
			Box box = new Box(numBox++, n, specialBoxes.contains(n));
			boxes.add(box);
		}
	}
