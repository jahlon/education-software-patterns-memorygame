...
import education.software.patterns.memorygame.model.EasyMemoryGame;
import education.software.patterns.memorygame.model.HardMemoryGame;
import education.software.patterns.memorygame.model.ImpossibleMemoryGame;
import education.software.patterns.memorygame.model.MediumMemoryGame;
...
public class MemoryGameApp {
	...
	/**
	 * Defines the difficulty levels of the game
	 * @author jehincapie
	 */
	...
	/**
	 * Creates a new instance of the model (the MemoryGame object)
	 * @param difficulty the difficulty of the game being created
	 */
	public void createNewGame(GameDifficulty difficulty) {
		if(game != null && !game.isBoardComplete() && game.getTimer().isRunning()) {
			game.getTimer().pause();
			boolean confirmation = MessageDialog.openConfirm(shell, 
										"Memory Game", 
										"Are you sure you want to cancel the current game and start a new one?");
			if(!confirmation) {
		...
		switch(difficulty) {
			case HARD:
				game = new HardMemoryGame();
				break;
			case MEDIUM:
				game = new MediumMemoryGame();
				break;
			case EASY:
				game = new EasyMemoryGame();
				break;
			case IMPOSSIBLE:
				game = new ImpossibleMemoryGame();
				break;
			default:
				break;
		}
		
		Observable timer = (Observable)game.getTimer();
		timer.addObserver(shell.getInfoPanel());
		timer.addObserver(shell);
		shell.loadGameBoard(game.getBoard());
		shell.updateScore(game.getScore());
		shell.updateTime(game.getSecondsRemaining());
		shell.unlockBoard();
	}
	...
	/**
	 * Receives the request that is sent when the countdown timer reaches 0
	 */
	public void timesUp() {
		shell.lockBoard();
		game.finish();
		shell.updateScore(game.getScore());
		shell.showGameMessage("Time's up!");						
	}
